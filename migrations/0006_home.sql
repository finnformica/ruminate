...
--
-- Every block remembers the note it was written in: `home_id`.
--
...
-- through it. `home_id` gives every block a note of its own regardless of
-- links, so a block that has fallen out of reach still has somewhere to be
-- shown: its home note's **Unassigned** basket (docs/graph-schema-v2.md,
...
-- not where it lives). Pages have no home. NULL on rows older than this
-- migration until the backfill below homes them.
--
-- Backfill: home every live block to the first page (by id) that reaches it
...
-- stays unhomed — nothing to show it under.
--
...
-- (CACHE_GENERATION, src/data/database-mode.ts), so the homes computed here
...
-- other index here.
CREATE INDEX nodes_tenant_home ON nodes (user_id, home_id);
...
UPDATE nodes
   SET home_id = (SELECT MIN(page) FROM reach
                   WHERE reach.user_id = nodes.user_id AND reach.node = nodes.id)
 WHERE type != 'page' AND home_id IS NULL
