...
import { getBlockType, leadingMarker, stripMarker, toggleTodo } from "../../blocks/block-type"
...
export function BlockItem({
  ...
  // The chevron (`BlockToggle`, block-marker.tsx) and the marker slot that
  // hosts it (`BlockMarker`) are the shared drawing every block view uses;
  // this row only decides what they do.
  const toggle = hasToggle ? (
    <BlockToggle
      collapsed={isCollapsed}
      beside={toggleBeside}
      onToggle={() => api.toggleCollapse(block.id)}
    />
  ) : null
  ...
  // toggle, so zoom stays on F / Cmd+. there.
  ...
  const marker = (
    <BlockMarker
      ...
      onZoom={zoomable ? () => api.zoomInto(block.id) : undefined}
      readOnly={readOnly}
      onToggleTodo={() => api.onContentChange(block.id, toggleTodo(block.content))}
    />
  )
    ...
    >
      <div className="relative min-w-0 py-0.5 font-content leading-relaxed">
        ...
        >
          {marker}
          {toggleBeside ? (
            // The beside toggle (see `BlockToggleBeside`): the top offset
            // mirrors the line's own vertical padding.
            <BlockToggleBeside className={cx(runEdges?.top ? "top-1" : "top-0.5", typo)}>
              {toggle}
            </BlockToggleBeside>
          ) : null}
