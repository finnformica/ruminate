 ...
 * arrives and leaves with the app's own motion. The close control is inside
 * the popup, which Base UI asks for so that a touch screen reader can escape
 * the dialog.
 */
  ...
  return (
    <BaseDialog.Portal>
      <BaseDialog.Popup
        ...
        className={cx(
          "fixed left-1/2 top-3 z-modal grid max-h-[75vh] w-[calc(100vw-24px)] max-w-md -translate-x-1/2 grid-rows-[auto_1fr] overflow-hidden focus:outline-hidden sm:top-[10vh]",
          className,
        )}
        {...props}
      >
