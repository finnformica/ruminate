 ...
 * (`SettingsNavList`, drawn by src/components/nav-items.tsx). The pages'
 * cards are in settings-pages.tsx; this module is only what names them, so
...
export interface SettingsPage {
  id: SettingsPageId
  label: string
  /** The line under the page's heading, and under its row on a phone — the
   * one line of descriptive copy a page carries (docs/settings.md). */
  description: string
...
const SETTINGS_PAGES: SettingsPage[] = [
  ...
  },
  {
    id: "preferences",
    label: "Preferences",
    description: "Theme and accent colour, how notes open, and the changelog.",
  },
  {
    id: "sharing",
    label: "Sharing",
...
/**
 * The pages as sidebar rows, in the sidebar's own recipe (`.nav-item`) and
 * at its size, for the place the Views list holds while Settings is open
 * (src/components/nav-items.tsx). The admin's page is set off beneath a
 * rule, the sidebar's own way of setting a row apart.
 */
export function SettingsNavList({ size }: { size: "medium" | "large" }) {
  const pages = useSettingsPages()
  return (
    <>
      <SettingsNavRows pages={pages.filter((page) => !page.admin)} size={size} />
...
function SettingsNavRows({ pages, size }: { pages: SettingsPage[]; size: "medium" | "large" }) {
  return (
