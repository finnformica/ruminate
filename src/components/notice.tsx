 ...
 * on the right.
 *
 * Variants color the icon rank, not the prose: `info` keeps a tertiary icon,
...
export const Notice = React.forwardRef<
  HTMLDivElement,
  {
    ...
    /** Replaces the variant's default icon (pass `null` to omit it). */
...
>(function Notice({ variant = "info", icon, actions, onDismiss, className, children }, ref) {
        ...
        {resolvedIcon !== null ? (
          <span
            className={cx(
              ...
              variant === "warning" ? "text-text-pending" : "text-text-tertiary",
            )}
          >
            {resolvedIcon}
          </span>
