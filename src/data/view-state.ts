 ...
 * set of collapsed occurrence keys per note in localStorage — collapsed means
 * folded, everything else is open — so a row can never hold two opinions at
 * once. Folds key by occurrence (`src/blocks/view.ts`): a block that shows up
 * twice in a note is two rows, folded independently.
 *
 * The default-expansion policy (`defaultCollapsedKeys`: headings always
 ...
 * has so dead keys don't accumulate. An empty set is still written: the entry
 * existing is what records "this note has been seeded here", so unfolding
 * everything survives a reload instead of inviting a re-seed.
 */
export function writeCollapsedKeys(noteId: string, collapsed: ReadonlySet<string>, doc: BlockDoc) {
  ...
  try {
    localStorage.setItem(
      storageKey(noteId),
      JSON.stringify([...collapsed].filter((key) => hasOccurrence(doc, key))),
    )
  } catch {
...
function resolve(noteId: string | undefined, doc: BlockDoc): CollapseState {
  ...
  return { noteId, seeded: true, collapsed: new Set(defaultCollapsedKeys(doc)) }
}
