...
- Every block now carries a marker, and collapsing is done from it. Bullets and to-dos show a dot, headings a `#`, numbered items their number, paragraphs a `¶` and quotes a `>`, all in one column, so text lines up across every block type. Hover the marker of a block that has children and it turns into a collapse arrow; click it to fold or unfold the block. The arrow only appears when your pointer is over the marker, so reading and editing a block never surfaces any controls. The indent guide line for the children hangs straight down from that marker rather than from a separate column of arrows to the left, so the outline reads as one structure and markers never sit off the lines. A folded block keeps its arrow showing so you can always see where content is hidden. A to-do's checkbox sits just after its dot, so ticking and folding never get in each other's way. On touch screens the arrow and checkbox both have larger tap areas. Clicking a bullet or number still zooms into a block that has no children; for a block with children, zoom with <kbd>F</kbd> or <kbd>⌘.</kbd>.
