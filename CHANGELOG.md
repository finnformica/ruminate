...
- Copying a block and pasting it onto another block in the **same** note now links it there, exactly as pasting into another note always has: the note holds the one block in two places, and editing either row changes both. It used to make an independent copy. Pasting onto a block that already holds the copied block directly beneath it still does nothing, and pasting a block inside itself still makes a copy rather than a loop. Pasting while typing inside a block still inserts the copied text.
