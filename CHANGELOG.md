...
- Loops. A block can now be pasted beneath one of its own descendants, or beneath itself several levels down: the outline shows the loop where it closes, as a row marked ↻ with nothing beneath it, and zooming into that row goes round once more. Only pasting a block directly onto itself is refused. Until now such a paste silently made a copy instead.
