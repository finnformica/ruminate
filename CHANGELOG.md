...
- Right-click any block for its menu. It carries the standard actions the keyboard already runs — **Edit**, **Turn into** (a submenu of Text, Bullet list, Numbered list, To-do, Heading, Quote and Code, with the block's current type marked), **Indent**, **Outdent**, **Move up**, **Move down**, **Duplicate**, **Collapse**/**Expand** on a block with children, **Zoom into**, **Copy** and **Copy link to block** — each with its shortcut shown beside it. Deleting is aware of shared blocks: on a block that appears in only one place, **Delete** removes it as before; on a block held in more than one place the menu shows how many, and offers **Remove from here**, which takes it out of this note only, and **Delete everywhere**, which removes the block itself from every place it appears. Right-clicking a row selects it first, so the menu always acts on the block under the pointer.
