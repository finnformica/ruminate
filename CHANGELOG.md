...
- Installed to an iPhone's home screen, the app fills the screen again: the phone nav bar sits at the bottom rather than floating above a band of empty space. iOS sizes the "small viewport" height as if Safari's toolbar were still there even in a home-screen app, so the frame stopped short; installed, the app now uses the plain viewport height instead. In a browser tab nothing changes.
