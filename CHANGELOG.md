...
- Sync now works block by block instead of note by note. Editing different parts of the same note on two devices no longer makes the later save overwrite the whole note, only edits to the very same block still resolve to the most recent save. The warning before a stale screen can overwrite newer content is unchanged.
