...
describe("search and tags", () => {
  ...
  it("takes the app's query language: a qualifier FILTERS, free text matches", async () => {
    ...
    // A note-level qualifier filters by the containing note.
    const tagged = await run(harness, grant, "search", { query: "tag:home" })
    expect(tagged.hits.length).toBeGreaterThan(0)
    expect(tagged.hits.every((hit: any) => hit.noteId === BETA)).toBe(true)
  ...
  it("says whether the semantic half was consulted", async () => {
    // With no bindings there is no semantic half, and `search` says so rather
    // than pretending or failing.
    const data = await run(harness, grantOf({}), "search", { query: "bullet" })
