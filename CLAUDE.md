...
- **Changelog** (docs/changelog.md): `CHANGELOG.md` is rendered by the app as well as read on GitHub — the `/changelog` page walks the releases parsed by `src/utils/changelog.ts`, which is also the CI gate's parser.
