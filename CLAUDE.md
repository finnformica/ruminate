...
- **Views** (`src/data/views.ts`, docs/metadata.md): every note is a view, and a block becomes one by having a view row (**Add to Views**). The sidebar, the Views page and the palette draw one list of both (`viewEntriesAtom`), ordered by the sort menu and, in the manual sort, by the view rows' `sort_key`. There is no pinning.
...
- **Routing**: TanStack Router, file-based under `src/routes/` (`routeTree.gen.ts` is generated by the Vite plugin; never edit it by hand). The home page (`/`) is the **Views** page; a note opens at `/views/<id>` (`/notes/<id>` redirects there, for old links).
