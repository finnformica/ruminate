...
- `collapse:<noteId>` localStorage entries orphan
  (`src/data/view-state.ts:23`).
...
| localStorage              | `collapse:<noteId>` (`view-state.ts:23`)                                                                                      | Old keys orphan; collapse state is disposable by design (`view-state.ts:5-18`) — accept re-seeding from the default policy, no rekey machinery. Per-note editor width lives in frontmatter (`_appRoot.notes_.$.tsx:229-241`), so it travels with the page; the panel-layout storage (`app-layout.tsx:23-27`) is not note-keyed. |
