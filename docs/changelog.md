...
**The card can be turned off.** Settings → Updates, **Show what's new after an
update** (`showWhatsNewAtom`, stored on the device like the card's own notes).
Off, the boot still takes the update request and records the build, so
switching it back on later does not greet the reader with a release they have
been running for weeks: not wanting to be told counts as read, exactly as
dismissing does. The `/changelog` page is unaffected — **What's new** in the
sidebar reaches it either way.
