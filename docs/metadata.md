...
**Views** is the sidebar's one list of your own — every note and every block view, under one heading, above what other people shared (**Shared**) — and it is the Views page (`/`) beneath **Recent**, and the ⌘K palette's Views group with nothing typed. A block view's row wears the focus glyph in the slot a note's favicon takes: it opens its note focused on the block, and that glyph is what focusing wears elsewhere. Everywhere the block editor draws the row — the Views page, the palette, a search result — a block view is a row like any other block's, with the marker that says what it is; nothing about the row itself says it is a view, because the list is where that shows.
