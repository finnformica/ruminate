...
- **Remove link**, which takes the link off and leaves its text as words
  (on a link block's card, the whole row goes). Beside it,
  **Turn into block** (or, on a link block's card, **Turn into inline**).
...
open while the pointer wanders — off the link, over the row beneath — and
closes only on a click outside, <kbd>Esc</kbd>, or your own action, as a
menu would. While any panel is open, no other link's card opens on hover.
...
picture on the left when it has one — a third of the card, never narrower
than a thumbnail is legible at, never more than half. The byline and the picture are links
