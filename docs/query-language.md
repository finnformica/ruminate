...
Each result is drawn exactly as the block is in its note — the same marker (dot, `#`, number, checkbox, `>`), the same type scale for its depth, the same collapse chevron in the marker slot — with only the breadcrumb added to say where it lives.
